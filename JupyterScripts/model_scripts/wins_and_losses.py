...
scenario_dict = {"20220127": "II", "20220412": "III", "20210417": "I"}


def add_bar(index_value, count_value, color, ax=plt, legend_label="p>0.05", bar_width=1):
    ax.bar(index_value, count_value, color=color,
           label=legend_label, width=bar_width)

class WinsAndLosses:
    def __init__(self, data, path, colors, fontsize=16, axes_label_fontsize=16, figsize=(8, 8), bar_width=.5, y_label_x = 0):
        self.data = data
        self.colors = colors
    ...
    def plot(self, title=None, ylabel_wins="Scenarios Won [%]", ylabel_losses="Scenarios Lost [%]", yticks=True, legend=True):
        self.fig, self.axs = plt.subplots(
            2, 1, figsize=self.figsize, sharex=True, gridspec_kw={'height_ratios': [self.y_max_win, self.y_max_loss]})
        ...
        if legend:
            ...
            colors = [self.colors["single_win"], self.colors["total_win"], self.colors["single_loss"], self.colors["total_loss"]]
            patches = [plt.Rectangle((0,0),1,1, color=color) for color in colors]
            self.axs[0].legend(patches, labels, loc='right', ncol=1, fontsize=self.fontsize, frameon=True)
    ...
    def barplot_wins(self, ax, ylabel, title, yticks):
        ...
        df.plot(kind='bar', ax=ax, width=self.bar_width, color=[self.colors["single_win"], self.colors["total_win"]])
        if ylabel:
    ...
    def barplot_losses(self, ax, ylabel, yticks):
        ...
        df.plot(kind='bar', ax=ax, width=self.bar_width, color=[self.colors["single_loss"], self.colors["total_loss"]])
        ax.set_ylabel(ylabel, fontsize=self.axes_label_fontsize)
            ...
            if strategies:
                if len(strategies) > 1:
                    ...
                    for strategy in strategies:
                        annotations_shared.update(
                            {strategy: scenario_label})
                else:
    ...
    def annotate(self, ax, y, dx = .13):
        for idx, strategy in enumerate(self.strategies):
            y_shared = y
            annotations_single = self.annotations_single.get(strategy, "")
            if ax:
                ax.annotate(annotations_single, xy=(idx, 0), xytext=(idx-dx, y),
                            fontsize=self.fontsize*0.9, fontweight='bold', ha='center', va='center', color='black')
            annotations_shared = self.annotations_shared.get(strategy, "")
            if ax:
                ax.annotate(annotations_shared, xy=(idx, 0), xytext=(idx+dx, y_shared),
                            fontsize=self.fontsize*0.9, fontweight='bold', ha='center', va='center', color='black')
    ...
    def plot_params(self, ax):
        ...
        if ax.get_ylim()[0] < 0:  # For losses plot
            ax.add_patch(patches.FancyArrowPatch((-.6, -self.y_max_loss), (-.6, -1),
                         arrowstyle='<-', color='black', mutation_scale=15))
        else:  # For wins plot
            ax.add_patch(patches.FancyArrowPatch((-.6, 1), (-.6, self.y_max_win),
                         arrowstyle='->', color='black', mutation_scale=15))
    ...
    def save(self, path, name_add=""):
        plt.savefig(os.path.join(path, "wins_and_losses"+name_add+".pdf"), dpi = 300)
