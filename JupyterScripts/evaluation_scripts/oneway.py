...
class OneWaySignificance:
    def __init__(self, df, group_col, variable, alpha=0.05):
        ...
        if self.anova_pvalue < alpha:
            ...
            self.tukey_df = pd.DataFrame(
                self.tukey_result.summary().data[1:],
                columns=self.tukey_result.summary().data[0],
            )
    ...
    def anova(self):
        ...
        SS_between = sum(
            len(self.df[self.df[self.group_col] == group])
            * (
                self.df[self.df[self.group_col] == group][self.variable].mean()
                - mean_total
            )
            ** 2
            for group in groups
    ...
    def make_cld(self):
        sorted_by_mean = (
            self.df[[self.group_col, self.variable]]
        ...
        groups = list(sorted_by_mean.index)
        ...
        for row in df.itertuples():
            if df["p-adj"][row[0]] > (self.alpha):
                cld.iat[groups.index(df["group1"][row[0]]), 2] += cld.iat[
                    groups.index(df["group2"][row[0]]), 1
                ]
                cld.iat[groups.index(df["group2"][row[0]]), 2] += cld.iat[
                    groups.index(df["group1"][row[0]]), 1
                ]

            if df["p-adj"][row[0]] < (self.alpha):
                cld.iat[groups.index(df["group1"][row[0]]), 3] += cld.iat[
                    groups.index(df["group2"][row[0]]), 1
                ]
                cld.iat[groups.index(df["group2"][row[0]]), 3] += cld.iat[
                    groups.index(df["group1"][row[0]]), 1
                ]
            ...
            for kitem in cld[1]:
                if kitem in item:
                    ...
                    if (
                        len(
                            set(
                                cld["labels"].loc[cld[1] == kitem].iloc[0]
                            ).intersection(cld.loc[cld[2] == item, "labels"].iloc[0])
                        )
                        <= 0
                    ):
                        if letters[g] not in list(
                            cld["labels"].loc[cld[1] == kitem].iloc[0]
                        ):
                            cld["labels"].loc[cld[1] == kitem] += letters[g]
                        if letters[g] not in list(
                            cld["labels"].loc[cld[2] == item].iloc[0]
                        ):
    ...
    def plot(
        ...
        if order:
            ...
            df = self.df.copy()
            df[order_col] = pd.Categorical(
                df[order_col], categories=order, ordered=True
            )
        ...
        means = (
            self.df[[self.group_col, self.variable]].groupby([self.group_col]).mean()
        )
        ...
        if self.anova_pvalue < self.alpha:
            self.add_tukey(
                ...
                means,
                bar_color,
                base_offset * df[self.variable].max(),
                fontsize_small,
                x_offset=dx,
            )
        return ax
    ...
    def add_tukey(self, ax, means, color, base_offset, fontsize, x_offset):
        if hasattr(self, "letters"):
            for xtick in ax.get_xticks():
                ...
                ax.text(
                    xtick + x_offset,
                    means.loc[label, self.variable] + offset,
                    string,
    ...
    def write_anova_results(self, filepath, decimal_places=3):
        modified_df = self.anova_result.applymap(
            lambda x: "$< 0.001$" if isinstance(x, float) and x < 0.001 else x
        )
    ...
    def write_tukey_results(self, filepath, decimal_places=3):
        self.tukey_df.to_latex(
            filepath, index=False, float_format=f"{{:0.{decimal_places}f}}".format
        )
