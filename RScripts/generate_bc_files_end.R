################################################################################
# Script to make barcode files for last transfer for reading in EvoWare        #
#                                                                              #
# usage:                                                                       #
...
# set up logging
scriptName <- "generate_bc_files_end"
...
# generate barcode file for assay plates
gen_barcodefile(barcodes, barcode_file_path,
  "bc_last_assay", args$transfer_n, args$run_n, 1, "assay")

# generate barcode files for agar plates to be spotted
for (iplate in 1:experiment$n_plates){
  gen_barcodefile(barcodes, barcode_file_path,
    str_c("bc_agar_", iplate), args$transfer_n, "max", iplate, "agar")
}
# generate barcode files for agar plates to be read
if (args$transfer_n > 1){
  # source plates
  gen_barcodefile(barcodes, barcode_file_path,
    "bc_last_agar_source", args$transfer_n - 1, "max", 1, "assay")
  # agar plates
  for (iplate in 1:experiment$n_plates){
    gen_barcodefile(barcodes, barcode_file_path,
      str_c("bc_last_agar_", iplate), args$transfer_n - 1, "max", iplate, "agar")
  }
}
